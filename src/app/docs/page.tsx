      ...
      <main className={styles.main}>
        <header className={styles.header}>
          ...
          <p className={styles.desc}>
            Learn how to deploy the WebSocket hooks, ingest Server-Sent Events from the Terminal, and leverage the exact mathematics behind the aviatorIQ prediction array.
          </p>
        </header>
        ...
        <section className={styles.section}>
          ...
          <p className={styles.text}>
            Data streams are emitted down a normalized `wss://` port at 50ms intervals. The aviatorIQ predictive engine intercepts these fragments and routes them to your local API routing edge. From there, the Neural model runs local inference to determine probabilistic crash intervals.
          </p>
          ...
          <div className={styles.codeBlock}>
            <div className={styles.codeHeader}>
              ...
              <button className={styles.copyBtn} onClick={() => handleCopy('api')}>
                {copiedApi ? 'COPIED!' : 'COPY REQUEST'}
              </button>
            </div>
            <pre className={styles.pre}>
              <code className={styles.code}>
                fetch(<span className={styles.tokenString}>&apos;/api/predict?provider=spribe&amp;limit=200&apos;</span>)<br/>
                .<span className={styles.tokenFunction}>then</span>(res =&gt; res.<span className={styles.tokenFunction}>json</span>())<br/>
                .<span className={styles.tokenFunction}>then</span>(prediction =&gt; console.<span className={styles.tokenFunction}>log</span>(prediction.safe_cashout));
              </code>
            </pre>
          </div>
          ...
          <div className={styles.codeBlock}>
            <div className={styles.codeHeader}>
              <span>POST /api/add</span>
            </div>
            <pre className={styles.pre}>
              <code className={styles.code}>
                fetch(<span className={styles.tokenString}>&apos;/api/add&apos;</span>, {'{'}<br/>
                &nbsp;&nbsp;method: <span className={styles.tokenString}>&apos;POST&apos;</span>,<br/>
                &nbsp;&nbsp;headers: {'{'} <span className={styles.tokenString}>&apos;Content-Type&apos;</span>: <span className={styles.tokenString}>&apos;application/json&apos;</span> {'}'},<br/>
                &nbsp;&nbsp;body: JSON.<span className={styles.tokenFunction}>stringify</span>({'{'} multiplier: 2.4, provider: <span className={styles.tokenString}>&apos;spribe&apos;</span> {'}'})<br/>
                {'}'})<br/>
                .<span className={styles.tokenFunction}>then</span>(res =&gt; res.<span className={styles.tokenFunction}>json</span>())<br/>
                .<span className={styles.tokenFunction}>then</span>(result =&gt; console.<span className={styles.tokenFunction}>log</span>(result.prediction));
              </code>
            </pre>
          </div>
        </section>
